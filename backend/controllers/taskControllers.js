...
exports.postTask = async (req, res) => {
  try {
    const { description, taskDate, taskDeadLine } = req.body;
    if (!description || !taskDate || !taskDeadLine) {
      return res.status(400).json({ status: false, msg: "All fields (description, taskDate, taskDeadLine) are required" });
    }
    const task = await Task.create({ user: req.user.id, description, taskDate, taskDeadLine });
    res.status(200).json({ task, status: true, msg: "Task created successfully.." });
  }
  catch (err) {
    console.error(err);
...
exports.putTask = async (req, res) => {
  try {
    ...
    task = await Task.findByIdAndUpdate(req.params.taskId, { description, taskDate, taskDeadLine }, { new: true });
    res.status(200).json({ task, status: true, msg: "Task updated successfully.." });
  }
  catch (err) {
    console.error(err);
