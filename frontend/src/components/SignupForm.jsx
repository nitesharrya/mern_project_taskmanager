...
const SignupForm = () => {
  ...
  const fieldError = (field) => (
    <p className={`error-message ${formErrors[field] ? "show" : "hide"}`}>
      <i className='mr-2 fa-solid fa-circle-exclamation'></i>
      {formErrors[field]}
    </p>
  );
  ...
  return (
    <form className="signup-form">
      {loading ? (
        ...
        <>
          <h2 className="form-title">Welcome , Signup Here</h2>
          ...
          <div className="login-link">
            <Link to="/login" className="login-text">Already have an account? Login here</Link>
          </div>
        </>
      )}
    </form>
