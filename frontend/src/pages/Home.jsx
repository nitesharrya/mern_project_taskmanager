...
const Home = () => {
  ...
  }, [isLoggedIn, user?.name]); // Include dependencies

  return (
    <MainLayout>
      {!isLoggedIn ? (
        <div className="home-guest">
          <h1 className="home-title">Welcome to Task Manager App</h1>
          <Link to="/signup" className="home-link">
            <span className="home-link-text">Join now to manage your tasks</span>
        ...
        <>
          <h1 className="home-welcome"><span className='span'>Welcome</span> {user.name}</h1>
          <Tasks />
        </>
      )}
    </MainLayout>
